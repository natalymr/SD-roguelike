...
from item_functions import heal, equip_base_armor, equip_base_weapon, drop, drop_attack_tool, drop_defense_tool
...
def place_entities(room, entities, max_monsters_per_room, max_items_per_room, colors):
    ...
    for i in range(number_of_items):
        ...
        if not any([entity for entity in entities if entity.x == x and entity.y == y]):
            if randint(1, 10) > 3:
                item_component = Item(use_function=heal, drop_function=drop, amount=4)
                item = Entity(x, y, '!', colors.get('violet'), 'Healing Potion', render_order=RenderOrder.ITEM,
                              item=item_component)
            else:
                if randint(1, 2) > 1:
                    item_component = Item(use_function=equip_base_weapon, drop_function=drop_attack_tool, amount=5)
                    item = Entity(x, y, '?', colors.get('red'), 'Base weapon', render_order=RenderOrder.ITEM,
                                  item=item_component)
                else:
                    item_component = Item(use_function=equip_base_armor, drop_function=drop_defense_tool, amount=3)
                    item = Entity(x, y, '?', colors.get('blue'), 'Base armor', render_order=RenderOrder.ITEM,
                                  item=item_component)
